...
# Automate pushing to Amazon S3 bucket
Sys.setenv("AWS_ACCESS_KEY_ID" = "AKIAJJ5FNW7P5DMT7MTQ","AWS_SECRET_ACCESS_KEY" = "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")
bucket_name <- bucketlist()[, 1][1]

zz <- rawConnection(raw(0), "r+")

write.csv(data_clean, zz, row.names = FALSE)

put_object(file = rawConnectionValue(zz),
           bucket = bucket_name, object = "UFC_Fighters_Database.csv")
close(zz)
