...
tweets_corpus <- tm_map(tweets_corpus, function(x) iconv(x, to='UTF-8-MAC', sub='byte'))
...
tweets_sentiment <- data.frame(as.integer(as.character(calculate_total_presence_sentiment(tweets_cleaned[[1]])[2, ])))

colnames(tweets_sentiment) <- c("count")

sentiment <- c("Sarcasm", "Neutral", "Negative", "Positive", "Very Negative", "Very Positive")

tweets_sentiment_data <- data.frame(temp, tweets_sentiment)

plot_ly(tweets_sentiment_data, x = ~sentiment, y=~count, type = "bar", color = ~sentiment)
